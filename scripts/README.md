...
| `package.json` deploy autopilots | Mainnet deployment | Convenience wrappers for the three runtime scripts: `npm run deploy:truffle:mainnet`, `npm run deploy:hardhat:mainnet`, `npm run deploy:foundry:mainnet`. They all consume `DEPLOY_CONFIG` and enforce the `$AGIALPHA` + governance invariants defined in [`scripts/deploy/load-config.js`](deploy/load-config.js). | `npm run deploy:<runtime>:mainnet` |
