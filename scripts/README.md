...
| [`owner-apply-validator-policy.js`](owner-apply-validator-policy.js) | Validator policy tuning | Loads a scenario manifest, computes deltas across `JobRegistry`, `StakeManager`, `ValidationModule`, and optionally `FeePool`, then relays a single `OwnerConfigurator.configureBatch` call through `SystemPause`. Streams governance telemetry (`ParameterUpdated`, `GovernanceCallExecuted`, module events) and can persist a Markdown summary for the evidence vault. | `truffle exec scripts/owner-apply-validator-policy.js --network <net> --manifest manifests/policy.json --summary manifests/governance/<date>-validator-policy.md` |
